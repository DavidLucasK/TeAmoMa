...
let points = 0;
let quizStatus = false; // Variável para armazenar o status do quiza
...
async function fetchQuestions() {
    try {
        const response = await fetch(`${backendUrl}/questions/2`, {
            method: 'GET',
            headers: {
...
async function fetchQuizStatus() {
    try {
        const response = await fetch(`${backendUrl}/quiz-status/1`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
...
async function updateQuizStatus() {
    try {
        const response = await fetch(`${backendUrl}/update-quiz-status/1`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            }
